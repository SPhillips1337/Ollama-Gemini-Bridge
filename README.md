...
A high-performance, Ollama-compatible HTTP bridge written in Python (FastAPI). This bridge allows you to use Gemini models with any Ollama-compatible client (Open WebUI, AnythingLLM, etc.) while leveraging your local **Gemini CLI OAuth session** or a direct API key.

## 🚀 Key Features

- **Ollama Spec Compatibility**: Implements `/api/chat`, `/api/generate`, and `/api/tags` to perfectly mimic the Ollama API.
...
- **Streaming Support**: Real-time NDJSON streaming for "typing" effects in UIs.
...
3. **Model**: Select `gemini-1.5-pro` (mapped internally to Gemini).
...
- **Streaming**: Ensure your client supports the NDJSON format used by Ollama.
